...
## ADR-0007 — Adopt `@tavily/core` SDK in the provider wrapper
**Status:** Accepted · **Date:** 2026-05-19
**Context:** ADR-0006 shipped crawl + search via a hand-rolled `fetch` wrapper plus four Zod schemas mirroring the Tavily response shapes. Both have first-party equivalents in `@tavily/core@0.7.3` (TS interfaces, axios-based client, camelCase options) that match exactly what we send and consume.
**Decision:** Replace the direct-`fetch` provider wrapper with the official SDK. Drop the four redundant Zod schemas and the `TAVILY_BASE` constant. Keep the three public functions (`extractUrlWithTavily`, `crawlBrandSite`, `searchBrandContext`) and the two helpers (`getTavilyPageContent`, `extractTavilyFailureReason`), all returning the existing `ProviderResult<T>` discriminated union. Test isolation now flows through an injected `TavilyClient` stub via a new `tavilyClient?` field on `ProviderOptions` — closer to how the code runs in production than the previous fetcher mock. Errors from the SDK (thrown promises) are caught and mapped to `status: "failed"` so `BrandDiscoverer` semantics are unchanged.
**Consequences:** `apps/web/src/lib/providers/tavily.ts` drops from ~300 lines to ~150. SDK types now drive the option/response surface so a Tavily API field change surfaces as a TS error at the call site, not as a silent Zod parse failure. `brand-discovery.ts` and its 7 tests are untouched — `BrandDiscoverer` was already the seam. Trade-offs: cannot mock by passing a `fetch` impl anymore (the SDK uses axios internally); test pattern is a one-method-per-test stub on the client interface. New dependency: `@tavily/core@0.7.3` (pinned), pulls axios + https-proxy-agent + js-tiktoken transitively.

## ADR-0008 — Product continuity moves to `sim0wastaken/motive`; this repo becomes the hackathon archive
**Status:** Accepted · **Date:** 2026-07-07
**Context:** After the hackathon, the productized GEO offering was built in a separate repo (`sim0wastaken/motive`): Next.js 15 + Convex + Clerk + Stripe, with a complete monetized loop (URL → live audit → $9/intent unlock → campaign kit). That repo has payments, auth, and a transactional engine this monorepo never had; this monorepo has product surfaces and domain depth that repo lacked. Maintaining two divergent stacks (Supabase/Inngest vs Convex) for one product is founder-mode waste.
**Decision:** `sim0wastaken/motive` is the product. The salvageable business/product surfaces of this repo were ported there on branch `claude/paris-hackathon-motive-port-4icsyb` (see `docs/PORT-PLAN.md` in that repo): fake-deploy → simulated launch + story-driven monitoring dashboard (`lib/motive/deployments.ts` + `performance.ts` → `src/lib/campaign.ts` + `convex/campaign*.ts`), creative-review HITL → ad-group HITL, the vertical-expert framework enums (Schwartz awareness, copy formulas, hook archetypes) into the kit-generation schema, plus the harness practices (CI gates, invariants doc, story-KPI and streaming invariants). Infrastructure did not port: Convex live queries replace Supabase Realtime, the Convex scheduler replaces Inngest, serializable mutations replace the `review_entity_action` RPC, and ScrapeGraph (already in motive) replaces Tavily. Pioneer remains permanently out of MVP scope. This repo is archived as the hackathon artifact and pattern reference (`report.md`); no further feature work lands here.
**Consequences:** Open items in `BLOCKERS.md`, `docs/exec-plans/tech-debt-tracker.md`, and `docs/post-hackathon/` are dispositioned against the port (resolved in motive, or permanently descoped) rather than fixed here. The append-only memory files stay frozen after this entry.
---

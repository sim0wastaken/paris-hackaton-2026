-- Spec 07 creative variant review support and asset error persistence.

alter table public.creative_variants
...
begin
  ...
  if p_entity_type not in ('extraction_run', 'brand_feature', 'conversation', 'landing_gap', 'ad_group', 'creative_variant') then
  ...
  case p_entity_type
    ...
    when 'creative_variant' then
      ...
      if invalid_keys is not null then
        raise exception 'invalid_review_patch: unsupported creative_variant fields %', invalid_keys;
      end if;

      select to_jsonb(cv.*) into before_row
      from public.creative_variants cv
      where cv.id = p_entity_id and cv.project_id = p_project_id
      for update;
      if before_row is null then
        raise exception 'review_entity_not_found';
      end if;
      ...
      update public.creative_variants
      set
        title = case when patch ? 'title' then patch->>'title' else title end,
        description = case when patch ? 'description' then patch->>'description' else description end,
        creative_angle = case when patch ? 'creative_angle' then patch->>'creative_angle' else creative_angle end,
        asset_type = case when patch ? 'asset_type' then (patch->>'asset_type')::public.creative_asset_type else asset_type end,
        asset_prompt = case when patch ? 'asset_prompt' then patch->>'asset_prompt' else asset_prompt end,
        asset_url = case when patch ? 'asset_url' then patch->>'asset_url' else asset_url end,
        asset_generation_status = case when patch ? 'asset_generation_status' then (patch->>'asset_generation_status')::public.asset_generation_status else asset_generation_status end,
        openai_file_id = case when patch ? 'openai_file_id' then patch->>'openai_file_id' else openai_file_id end,
        target_url = case when patch ? 'target_url' then patch->>'target_url' else target_url end,
        openai_ad_status = case when patch ? 'openai_ad_status' then patch->>'openai_ad_status' else openai_ad_status end,
        provider_request_json = case when patch ? 'provider_request_json' then provider_request_json || (patch->'provider_request_json') else provider_request_json end,
        provider_response_json = case when patch ? 'provider_response_json' then provider_response_json || (patch->'provider_response_json') else provider_response_json end,
        openai_validation_json = case when patch ? 'openai_validation_json' then openai_validation_json || (patch->'openai_validation_json') else openai_validation_json end,
        metadata = case when patch ? 'metadata' then metadata || (patch->'metadata') else metadata end,
        status = case
          when p_action = 'approve' then 'approved'::public.creative_status
          when p_action = 'reject' then 'rejected'::public.creative_status
          else status
        end,
        review_status = review_status_value
      where id = p_entity_id and project_id = p_project_id
      returning to_jsonb(creative_variants.*) into after_row;
  end case;
